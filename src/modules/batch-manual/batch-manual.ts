...
export class BatchManualModule implements IModule {
  ...
  private async extractForeignWords() {
    ToastService.showProgress(`Memindai kata asing di dokumen...`, 0);
  ...
  private execute(wholeDocument: boolean) {
    ...
    void WordService.processWithConfirmation(wholeDocument, async (range, isDryRun, token, onProgress, searchCache) => {
